...
def DB_Totales(ID):
    ...
    Registro = Realiza_consulta(mi_vs.BASE_VARIOS_PPAL, sql)
    Total = 0
    Activos = 0
    for i in Registro:
        Total = i[2]
...
def Dev_Info_Producto(Codigo):
    ...
    try:
        # Tabla: Productos
        Reg = Reg_Un_param(mi_vs.BASE_DATOS, "Productos", "Codigo", Codigo)
        for i in Reg:
        ...
        # Tabla: Stock
        Reg = Reg_Un_param(mi_vs.BASE_DATOS, "Stock", "ID", Id_)
        for i in Reg:
            Lista_Datos.append(i[1])
            Lista_Datos.append(i[2])
            Lista_Datos.append(i[3])
        ...
        # Tabla: Adicionales
        Reg = Reg_Un_param(mi_vs.BASE_DATOS, "Adicionales", "ID", Id_)
        for i in Reg:
            Lista_Datos.append(i[1])
            Lista_Datos.append(i[2])
            Lista_Datos.append(i[3])
...
def Dev_Columna_Lista(Tabla, Colum_Int):
    Tabla = Dev_Tabla(mi_vs.BASE_GENERAL_PPAL, Tabla)
    Lista = []
    for reg in Tabla:
